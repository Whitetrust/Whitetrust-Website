...
2. Add these five:
   ...
   - `User.Read.All`
   - `BookingsAppointment.Read.All` — lets the site read Microsoft Bookings
     appointments (customer name/email/phone, service, assigned staff) so
     they can be saved to the site's database alongside contact-form
     enquiries.
3. Click **Grant admin consent for {tenant}** → confirm. All five must
   read **"Granted for …"**.
...
# Include the Bookings business mailbox (WhiteTrust@cawt.ai) too — the
# site now also reads Microsoft Bookings appointment data from it.
New-DistributionGroup `
  -Name "CAWT-Booking-Mailboxes" `
  -Type "Security" `
  -PrimarySmtpAddress "cawt-booking-mailboxes@cawt.ai" `
  -Members "neerajaggarwal@cawt.ai","niyati@cawt.ai","chirag@cawt.ai","contact@cawt.ai","WhiteTrust@cawt.ai"
...
**Bookings business mailbox** — the UPN the "Book an appointment" pages use
(`WhiteTrust@cawt.ai` today; only mention this if it's ever different).
...
| `BOOKINGS_BUSINESS_ID` | Part B (Bookings business mailbox, defaults to `WhiteTrust@cawt.ai`) |
| `COSMOS_ENDPOINT` | Part C 🔑 (Cosmos account URI) |
| `COSMOS_KEY` | Part C 🔑 (Cosmos primary key) |
| `COSMOS_DATABASE` | `whitetrust` |
| `COSMOS_CONTAINER` | `enquiries` (contact-form enquiries) |
| `COSMOS_BOOKINGS_CONTAINER` | `bookings` (Microsoft Bookings appointments, auto-created) |
| `CRON_SECRET` | Generated by the website team, not IT — protects the booking-sync endpoint |
| `MONGODB_URI` | Optional 🔑 (only if analytics/prospect-tracking dashboard is used) |
| `MONGODB_DB` | `cawt` |
